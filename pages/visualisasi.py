...
def visualization():
        ...
        try:
            if 'at' not in data.columns:
                ...
                if data['at'].isnull().all():
                    ...
                    monthly_sentiment = valid_data.resample('ME', on='at')['Score'].mean().reset_index()
    ...
    with tab3:
        ...
        st.markdown("**Positif**")
        text_pos = " ".join(data[data['Sentiment'] == 'Positif']['stemming'])
        ...
        st.markdown("**Negatif**")
        text_neg = " ".join(data[data['Sentiment'] == 'Negatif']['stemming'])
        ...
        st.markdown("**Netral**")
        text_neg = " ".join(data[data['Sentiment'] == 'Netral']['stemming'])
        wordcloud_neg = WordCloud(width=800, height=400, background_color='white', max_words=500).generate(text_neg)
        plt.figure(figsize=(10, 5))
        plt.imshow(wordcloud_neg, interpolation='bilinear')
        plt.axis('off')
    ...
    with tab4:
        ...
        if 'stemming' in data.columns and 'Sentiment' in data.columns:
            ...
            positive_texts = data[data['Sentiment'] == 'Positif']['stemming'].tolist()
            negative_texts = data[data['Sentiment'] == 'Negatif']['stemming'].tolist()
            ...
            st.warning("Kolom 'stemming' atau 'Sentiment' tidak ditemukan dalam dataset")
